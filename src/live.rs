//! Rolling previews from the open capture, without touching the clipboard.
//!
//! pw-record finalises WAV lengths only on exit. We snapshot actual PCM bytes
//! into a separate, valid WAV and ask the warm daemon about the last 15 seconds.
//! A session token prevents delayed replies from leaking into another dictation.

use crate::{audio, ipc, notify, paths::Paths, state};
...
pub fn worker(paths: &Paths, session: &str, recorder: i32) -> Result<()> {
    ...
    while active(paths, session, recorder) {
        ...
        let frame = match snapshot_pcm(source) {
            Ok(Some(frame)) if frame.end > previous_end => frame,
            Ok(_) => continue,
            Err(error) => {
        ...
        // Silence and unchanged audio do not require a model invocation.
        previous_end = frame.end;
        if frame.pcm.iter().all(|&byte| byte == 0) {
            continue;
        }
        write_wav(&snapshot, &frame.pcm)?;
        let response = match ipc::preview(&paths.asr_socket(), &snapshot) {
        ...
        let preview = Preview {
            text: crate::sanitize(&response.text),
            language: response.language,
            provisional: true,
            window_start_seconds: frame.start as f64 / BYTES_PER_SECOND as f64,
            audio_seconds: frame.end as f64 / BYTES_PER_SECOND as f64,
            error: None,
        };
        publish(paths, &preview)?;
        if preview.text != last_text {
            notify::live(&preview.text, &mut notice_id);
...
fn snapshot_pcm(source: &mut File) -> Result<Option<Frame>> {
    source.rewind()?;
    ...
    loop {
        ...
        if &chunk[..4] == b"data" {
            ...
            let end = (length - data) & !1;
            if end < BYTES_PER_SECOND / 2 {
                return Ok(None);
            }
...
mod tests {
    ...
    fn snapshots_ignore_stale_lengths_and_keep_only_the_recent_window() {
        ...
        file.write_all(&0u32.to_le_bytes()).unwrap();
        let frame = snapshot_pcm(&mut file).unwrap().unwrap();
        ...
        assert!(snapshot_pcm(&mut file).unwrap().is_none());
    }
}
